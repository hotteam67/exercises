...
/** The Talon we want to motion profile. */
	...
	/** joystick for testing class joystick object _joy*/
	HotJoystick _joy;
	...
	bool ButtonA = {false};
	//bool ButtonB = {false};
	//bool OldButtonB = {false};


	Robot() : _talon(Constants::kTalonID),  _example(_talon), _joy(0)
	{
	}
	...
	void TeleopPeriodic()
	{
		/* get buttons; [structured as] variableDeclaredInThisScope = object . functionInObject */
		ButtonA = _joy.ButtonA();
		//ButtonB = _joy.ButtonB();
		//bool OldButtonB();

		/*for(unsigned int i=1;i<10;++i)*/
			/*ButtonB() = _joy.GetRawButton;*/
		...
		if (ButtonA == false) { /* Check button a (top left shoulder on the logitech gamepad). */
			/*
			...
			//if( (ButtonB == true) && (OldButtonB == false) ) {
				/* user just tapped button b */
